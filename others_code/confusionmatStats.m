...
for class = 1:numOfClasses
    ...
    sensitivity(class) = TP(class) / (TP(class) + FN(class) + 1e-40);
    specificity(class) = TN(class) / (FP(class) + TN(class)+ 1e-40);
    precision(class) = TP(class) / (TP(class) + FP(class)+ 1e-40);
    f_score(class) = 2*TP(class)/(2*TP(class) + FP(class) + FN(class)+ 1e-40);
end
